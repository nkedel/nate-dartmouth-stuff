...
button .cmnd.startX   -width 8 -height 3 -text "Start as X" -command { start_as "x" }
button .cmnd.startO   -width 8 -height 3 -text "Start as O" -command { start_as "o" }
button .cmnd.autoPlay -width 8 -height 3 -text "AutoPlay"   -command { start_as "a" }
button .cmnd.quit     -width 8 -height 3 -text "Quit"        -command { exit }
...
while {1} { 
  ...
  if { [won3D $board] != "n" } {
    ...
    toplevel .win
    switch [ tk_dialog .win "Winner" $winner "" 0 "Restart as X" "Restart as Y" "AutoPlay" "Quit" ] 0 { start_as "x" } 1 { start_as "o" } 2 { start_as "a" } 3 { exit } 
    }
  }
