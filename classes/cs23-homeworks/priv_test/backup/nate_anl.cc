...
//    if (x == -2) break; 
  ...
  if ((worst <= 1) && (best >= 1)) bestmv = selectRandom(newBoard);
#ifdef DEBUG
...
int analyzeX(char *newBoard, int depth, int last) {
  int x, 
      rval = 3;
  if (wonAdj(newBoard, last)) return -2; 
...
//           recursively. 
//
// Returns: 2 if there is nothing visible within depth
//          1 if there is an opponent win within depth
//          0 if there is a tie within depth
//          -1 if there is a win/block within depth
//
